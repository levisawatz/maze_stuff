...
class Plotter:
    ...
    def plot(self,radar):
        ...
        self.ax1.bar(x[:-1],dr)
        self.ax1.legend(["Mean","Distance", "dDist/dAng"],loc=1)
...
class Man:
    def __init__(self,data):
        ...
        self.periferial=VIEW_ANGLE
        self.nrays=SIGHT_ACCURACY
        self.seelines=[]
    ...
    def seeman(self,data):
        ...
        if WANDERMODE==1: index=self.hugright()
        else: index=self.weighted_choice()[0]
...
def main():
    ...
    print(data.shape)
    men=[Man(data) for _ in range(N_MEN)]
    man=men[0]
    while True:
        ...
        rect.center=(DISPWIDTH//2,DISPHIGHT//2)
        disp.blit(mazething,rect)
        if SHOW_RADAR: displines(disp,(0,250,0),man.seelines)
        if SHOW_PATH: drawpath(disp,path,man.gridp)
